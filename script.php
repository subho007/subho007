...
@exec('adb-'.$platform.' devices');
exec('adb-'.$platform.' devices', $d);
$extend = "";
device:
$dev = 0;
/////////// Menu of devices!! ////////////////////////////////////
...
exec('adb-'.$platform.' -s '.$device.' root');
exec('adb-'.$platform.' -s '.$device.' shell ls -l '.$base, $m);
///////////////File list menu ////////////////////////////////////////////
menu:
...
if(empty($input) || $input=="\r" || $input>=$n || $input<=0 || !is_numeric($input)) { 
	...
	if(stristr($files[0],'d')) {
		...
		$m = "";
		exec('adb-'.$platform.' -s '.$device.' shell ls -l '.$base.$extend, $m);
		goto menu;
	}
	$file = $files[6];
}
...
if(file_exists($device."/".$file)) {
	echo "Remove the db file from the folder to copy !";
} else {
	exec('adb-'.$platform.' -s '.$device.' pull '.$base.$extend.$file.' '.$device);
}
goto menu;
findall:
...
exec('adb-'.$platform.' -s '.$device.' shell ls -l '.$base.$extend, $ma);
foreach ($ma as $line) {
	$slice = preg_split("/\s+/", $line, -1, PREG_SPLIT_NO_EMPTY);
	$end=7;
	if(stristr($slice[0],'d')) {
		$end=6;
		scaneer($base.$extend.$slice[$end-1],$platform,$device);
	}
	if($end==7) {
		if(stristr($slice[$end-1],'.db')) {
			exec('adb-'.$platform.' -s '.$device.' pull '.$base.$extend.$slice[$end-1].' '.$device);
		}
	}
}
goto menu;
/////////////////////// function to get all .db files inside a specific path ////////////////////////
function scaneer($path,$platform,$device) {
exec('adb-'.$platform.' -s '.$device.' shell ls -l '.$path, $mf);
foreach ($mf as $line) {
	$slice = preg_split("/\s+/", $line, -1, PREG_SPLIT_NO_EMPTY);
	$end=7;
	if(stristr($slice[0],'d')) {
		$end=6;
		scaneer($path.'/'.$slice[$end-1],$platform,$device);
	}
	if($end==7) {
		if(stristr($slice[$end-1],'.db')) {
			exec('adb-'.$platform.' -s '.$device.' pull '.$path.'/'.$slice[$end-1].' '.$device);
		}
	}
}
}
